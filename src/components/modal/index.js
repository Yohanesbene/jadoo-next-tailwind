...
export default function Modal({ id, children, wrapperClassName }) {
  ...
  if (modalContainer) {
    return createPortal(
      <CSSTransition timeout={150} in={isActive} unmountOnExit classNames={{
        appear: "opacity-0",
        ...
        <div className="transition-all fixed inset-0 bg-black/10 z-50 overflow-auto">
          <div className={['flex items-center justify-center min-h-screen', CONTAINER_MARGIN_Y].join(" ")}>
            <div className={['bg-white py-5 px-5 relative shadow-great rounded-md mx-4', wrapperClassName].join(" ")}>
              <button className="absolute right-0 top-0 w-8 h-8 flex items-center justify-center" onClick={() => {
                setActive(false);
                router.push({ href: "/" });
              }}>&times;</button>
              {children}
